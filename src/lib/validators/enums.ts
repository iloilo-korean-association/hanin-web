...
export const PERMISSIONS = [
  // ── 돈 ──────────────────────────────────────────────
  /**
   * ★ 예전 이름은 '승인권'(지출 결재·집행)이었다.
   *   사전 승인 절차를 없애면서 뜻이 바뀌었다 — 이제는 **장부에 적힌 뒤** 감사 확인 대기 큐에서
   *   "내가 봤다" 도장을 찍는 권한이다. 통제가 입력 전에서 입력 후로 옮겨졌기 때문이다.
   *   이름을 그대로 두면 승인 절차가 아직 있는 줄 알고 다음 총무가 결재를 기다린다.
   */
  "확인권", // 감사 확인 도장 (Transaction.reviewedBy)
...
export const MONEY_PERMISSIONS = ["확인권", "조회권", "입력권"] as const;
...
export const PERMISSION_HELP: Record<Permission, string> = {
  확인권:
    "감사 화면의 확인 대기 큐에서 거래에 확인 도장을 찍습니다. 장부 내용은 고칠 수 없고, 본인이 적은 거래는 본인이 확인할 수 없습니다.",
  조회권: "임원 화면을 열람합니다. 이것만 있으면 읽기 전용(감사)입니다.",
  입력권: "장부에 수입·지출을 적고, 고치고, 무효 처리합니다. 승인 절차 없이 즉시 확정됩니다.",
